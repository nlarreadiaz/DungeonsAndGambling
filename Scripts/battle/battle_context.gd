...
func _decorate_actors(raw_actors: Variant, side: String) -> Array:
	...
	for raw_actor in raw_actors:
		...
		var actor = {
			...
			"inventory_attack_bonus": max(int(raw_actor.get("inventory_attack_bonus", 0)), 0),
			"inventory_defense_bonus": max(int(raw_actor.get("inventory_defense_bonus", 0)), 0),
			"inventory_speed_bonus": max(int(raw_actor.get("inventory_speed_bonus", 0)), 0),
